...
$(window).load(function(){
	$("#magazine_pop").css({"height":($(window).height())+"px","top":($(document).scrollTop())+"px"});
...
function magazine_view(idx){
	jQuery.ajax({
		url: "/activity/ajax_magazine.php",
		type : "GET",
		data: "idx="+idx,
		...
		success: function(data){
			$("#magazine_pop").html(data).fadeTo(700,1, function(){
				var sh = $(".sliderkit-panel-active").eq(0).find("img").height();
				$(".photoslider-bullets3, .sliderkit-panel, .sliderkit-panels").css("height",sh+"px");
			});
			//	$("#magazine_pop").html(data).show();
			comment_load(idx,1);
		},
		complete: function(){
...
function comment_load(idx,page){
	jQuery.ajax({
		url: "/activity/comment_ajax.php",
		type : "GET",
		data: "idx="+idx+"&cpage="+page,
...
function magazine_close(){
	$("#magazine_pop").fadeTo(700,0);
	setTimeout(function(){ $("body,html").css({"overflow":"auto"}); $("#magazine_pop").hide(); }, 700);
}
