...
if(!$is_admin && !$is_manager && !empty($BoardDateRow["ViewAuthority"]) && $member["Level"] < $BoardDateRow["ViewAuthority"]){
...
//http://doctorsmi.com/online/counsel.php?board_code=board_write&BoardIdx=491&page=1&cat=&Category=&sF=&sT=&workType=real&mode=mk_board_counsel
...
<input type="hidden" name="board_idx" value="<?=$view[BoardIdx]?>">
...
?>
<div class="pt30">
	<table border="0" cellspacing="0" class="tb01">
		<tr>
			<td class="title"><?=!empty($view["Category"])?"[".$view["Category"]."] ":""?><?=$view["Title"]?></td>
		</tr>
		<tr>
			<td class="division">
				<table width="170" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td><?=substr($view["RegDate"],0,10)?></td>
						<td class="pl15"><img src="../image/story_img/title_line.gif" alt=""></td>
						<td class="pl15">조회수 <?=number_format($view["ReadNum"])?></td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<tr>
			<td>
				<div  class="answer">
					<p class="answer_tit">[답변] <?=$view["Title"]?></p>
					<?=nl2br($view["bd1"])?>
				</div>
			</td>
		</tr>
		...
		<tr>
			<td class="file">
				...
				//	if($files[$i][image_type]=="1" || $files[$i][image_type]=="2" || $files[$i][image_type]=="3" || $files[$i][image_type]=="6") continue;
					echo "<img src='../image/story_img/file_img.gif' alt='첨부파일' align='absmiddle' hspace='5'> <a href='".$view["files"][$i][href]."' class='bbs'> ".$view["files"][$i][file_name]."</a><br>";
				}
				?>
			</td>
		</tr>
		<tr>
			<td class="right pt10 pr10">
				<a href="javascript:;" onclick="<?=$modify_link_in_view?>"><img src="../image/story_img/modify_btn.gif" alt="수정" class="pr10"></a>
				<a href="javascript:;" onclick="<?=$delete_link_in_view?>"><img src="../image/story_img/delete_btn.gif" alt="삭제" class="pr10"></a>
				<a href="<?=$_SERVER['PHP_SELF']?>?board_code=board_list&page=<?=$page?>&<?=$searchVal?>" title="목록"><img src="../image/story_img/list_btn.gif" alt="목록"></a>
			</td>
		</tr>
	</table>
</div>	
