...
<div id="container">
	<div class="content_view">
		<div class="con_title"><?=$board_setting["BoardTitle"]?></div><? if($BoardName == "account"){ ?>
		<form name="info_form" method="get">
		...
		<table class="write_table">
			...
			<tbody>
			<tr>
				<th><label>예약기간</label></th>
				<td><input type="text" class="input datepick" name="sdate" style="width:70px;" title="시작일" value="<?=$_GET['sdate']?>" id="sdate"> ~ <input type="text" class="input datepick" name="edate" style="width:70px;" title="종료일" value="<?=$_GET['edate']?>" id="edate"></td>
				<th><label>예약자명</label></th>
				<td><input type="text" class="input" name="aname" id="aname" title="예약자명" value="<?=$_GET["aname"]?>"></td>
				<th><label>구분</label></th>
				<td>
					<select name="acat" id="acat">
						<option value=''>전체</option>
						<option value="간편예약" <?=$_GET["acat"]=="간편예약"?"selected":""?>>간편예약</option>
						<option value="온라인예약" <?=$_GET["acat"]=="온라인예약"?"selected":""?>>온라인예약</option>
					</select>
				</td>
				<th><label>담당의</label></th>
				<td>
					<select name="acharge" id="acharge">
						<option value=''>전체</option>
						<?
						$csql = " select * from ".$site_prefix."charge where 1=1 order by partorder desc, chargeorder desc, idx desc  ";
						$cresult = sql_query($csql);
						for($i=0;$crow = sql_fetch_array($cresult);$i++){
							if($_GET['acharge'] == $crow["idx"]) $csel = "selected";
							else $csel = "";
							echo "<option value='".$crow["idx"]."' ".$csel.">".$crow["name"]."</option>";
						}
						?>
					</select>
				</td>
				<td align=right><button type="button" class="btn_a_b" onclick="form_ck();">검 색</button></td>
		...
		<div class="btn_group">
			<button type="button" class="btn_a_n" onclick="excel_down();">엑셀다운</button>&nbsp;&nbsp;
		</div>
		</form>
		<? } ?>
		<table class="list_table mt15">
			<colgroup>
				<col width="5%">
				<?=$BoardName == "account"?"<col width='10%'>":""?>
				<col width="">
				<col width="12.5%">
				<col width="12.5%">
				<col width="10%">
			...
			<tr>
				<th>No</th>
				<?=$BoardName == "account"?"<th>구분</th>":""?>
				<th>제목</th>
				<th><?=$BoardName == "account"?"예약자명":"작성자"?></th>
				<th><?=$BoardName == "account"?"상태":"작성일"?></th>
				<th><?=$BoardName == "account"?"상태":"조회수"?></th>
			</tr>
			</thead>
			<tbody>
			<?
			...
			<tr>
				<td><?=$num?></td>
				<?=$BoardName == "account"?"<td>".$row["Category"]."</td>":""?>
				<td class="subject">
					<?
					if($BoardName == "account") echo "<strong>[".$bd4."]</strong>";
					...
					if($row["Category"] && $BoardName != "account"){ echo "<a href='".$_SERVER['PHP_SELF']."?bidx=".$bidx."&Category=".urlencode($row["Category"])."'>[".$row["Category"]."]</a>";}
					...
					if($row["bd1"] && $BoardName == "mainAfter") echo "&nbsp;&nbsp;<span style='color:#e00000;font-weight:bold;'>[메인노출]</span>";
					?>
				</td>
				<td><?=$username?></td>
				<td>
					...
					if($BoardName == "account"){
						switch($row["bd10"]){
							...
							case "Y":
								...
								break;
							default:
								if($row["Category"] == "간편예약")
									echo "예약문의";
								else 
									echo "예약대기";
						}
					} else {
						echo substr($row["RegDate"],0,10);
					}
					?>
				</td>
				<td>
					<?
					if($BoardName == "account"){
						if($row["bd10"] == "Y") echo " <span style='color:#e00000;font-weight:bold;'>[취소됨]</span>";
						else if($row["Category"] == "온라인예약" && $row["bd10"] == "N") echo " <a href='javascript:;' onclick='account_cancle(\"".$row["BoardIdx"]."\",\"D\");'><span style='color:#e00000;'>[취소하기]</span></a><br><a href='javascript:;' onclick='account_cancle(\"".$row["BoardIdx"]."\",\"E\");'><span style='color:#335ecc;'>[예약완료]</span></a>";
						if($row["bd8"]){
							echo "<br>".$row["bd8"]."<br>".$row["bd9"];
						}
					} else {
						echo $row["ReadNum"];
					}
...
function excel_down(){
	if($("#acat").val() == ""){
		alert("검색창의 구분항목에서 간편예약 또는 온라인예약을 선택한뒤 다운받아주시기 바랍니다.");
		return;
	}

	location.href = "account_excel.php?sdate="+$("#sdate").val()+"&edate="+$("#edate").val()+"&aname="+$("#aname").val()+"&acat="+$("#acat").val()+"&acharge="+$("#acharge").val();
}
function account_cancle(idx,type){
	switch(type){
		case "D":
			...
			break;
		case "E":
			if(!confirm("완료된 자료는 되돌릴 수 없습니다. 예약완료 처리하시겠습니까?")) return;
			location.href = "/board/admn/_proc/board/_board_proc.php?workType=AE&idx="+idx;
			break;
	}
}
