			...
			<tr>
				<td width="100%" valign="top">
					...
					<table class="write_table">
						...
						<tr>
							<th><label>의료진 노출순서</label></th>
							<td>
								<input type="text" class="input" name="chargeorder" exp title="의료진 노출순서" value="<?=$row["chargeorder"]?>">
								<span style="margin-left:20px;color:#e00000;font-weight:bold;">※ 숫자가 클수록 예약순서에서 먼저 출력됩니다.</span>
							</td>
						</tr>
						<!--tr>
					...
					<table class="list_table mt15">
						...
						<tbody>
						<?
						$sql = " select * from ".$site_prefix."charge where 1=1 order by partorder desc, chargeorder desc, idx asc ";
						$result = sql_query($sql);
						for($i=0;$row = sql_fetch_array($result);$i++){
						?>
						<tr>
							<td><?=$row["part"]?></td>
							<td><?=$row["name"]?> <span style="color:#e00000;font-weight:bold;">[노출순서 : <?=number_format($row["chargeorder"])?>]</span></td>
							<td><?=$row["etc"]?></td>
							<td>
