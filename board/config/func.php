...
function get_paging_mobile($write_pages, $cur_page, $total_page, $url, $add=""){ //보여줄 개수, 현재페이지, 총페이지, 주소, 추가
    $str = "";
    if ($cur_page > 1) {
        $str .= "<li><a href='" . $url . "1{$add}'class='first'>처음</a></li>";
        //$str .= "[<a href='" . $url . ($cur_page-1) . "'>이전</a>]";
    }
    ...
    if ($start_page > 1) $str .= "<li><a href='" . $url . ($start_page-1) . "{$add}' class='prev'>이전</a></li>";
    ...
    if ($total_page > $end_page) $str .= "<li><a href='" . $url . ($end_page+1) . "{$add}' class='next'>다음</a></li>";

    if ($cur_page < $total_page) {
        //$str .= "[<a href='$url" . ($cur_page+1) . "'>다음</a>]";
        $str .= "<li><a href='$url$total_page{$add}' class='last'>맨끝</a></li>";
    }
    $str .= "";
...
function get_naver_api($address){
	$url = "https://openapi.naver.com/v1/map/geocode?encoding=utf-8&coord=latlng&output=json&query=".urlencode($address);

	$is_post = false;
	
	if(preg_match("/mv\-web/i",$_SERVER["HTTP_HOST"])){
		$client_id = "jI7cz0CFsUb5KgPcJ34u";
		$client_secret = "BdT6clBefQ";
	} else {
		$client_id = "YsANpM108a15O_lgV4hj";
		$client_secret = "fBKBQfGAHC";
	}
