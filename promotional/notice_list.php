<?php require_once($_SERVER['DOCUMENT_ROOT'].'/inc/dochead.php'); ?>
<link href="/assets/css/sub.css" rel="stylesheet" />
<link href="/assets/css/promotional.css" rel="stylesheet" />
</head>
<body>
	<?php require_once($_SERVER['DOCUMENT_ROOT'].'/inc/header.php'); ?>
	<div class="page-header page-header-1">
		<div class="container">
			<ol class="breadcrumb">
				<li><a href="#"><span class="glyphicon glyphicon-home"></span></a></li>
				<li><a href="#">홍보센터</a></li>
				<li class="active">보도자료</li>
			</ol>
			<hr>
			<h1>보도자료</h1>
			<p>언론에 소개된 다비육종</p>
		</div>
	</div>
	<main id="content" tabindex="-1" class="promotional notice-list">
		<div class="container">
			<div class="search-area" style="margin:30px 0;">
                <form>
                    <div class="form-group">
                        <select class="form-control">
                            <option value="title">제목</option>
                            <option value="content">내용</option>
                            <option value="username">작성자</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="search-keyword" class="sr-only">검색어</label>
                        <input id="search-keyword" type="text" class="form-control">
                    </div>
                    <button type="submit" class="btn"><span class="glyphicon glyphicon-search"></span></button>
                </form>
            </div>
            <div class="table-wrap">
                <table class="table table-border">
                    <colgroup>
                        <col style="width: 150px;">
                        <col style="width: 570px;">
                        <col style="width: 150px;">
                        <col style="width: 150px;">
                        <col style="width: 150px;">
                    </colgroup>
                    <thead>
                        <tr>
                            <th>번호</th>
                            <th>제목</th>
                            <th>작성자</th>
                            <th>등록일</th>
                            <th>조회수</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td class="notice">공지</td>
                            <td class="notice subject">
                                <a href="#">
                                    제5회 『건강한 다비퀸』 돼지 사진 공모전제5회
                                </a>
                            </td>
                            <td>다비육종</td>
                            <td>2016-01-01</td>
                            <td>100</td>
                        </tr>
                        <tr>
                            <td>10</td>
                            <td class="subject">
                                <a href="#">
                                    제4회 『건강한 다비퀸』 돼지 사진 공모전 결과
                                </a>
                            </td>
                            <td>다비육종</td>
                            <td>2016-01-01</td>
                            <td>100</td>
                        </tr>
                        <tr>
                            <td>9</td>
                            <td class="subject">
                                <a href="#">
                                    제4회 『건강한 다비퀸』 돼지 사진 공모전
                                </a>
                            </td>
                            <td>다비육종</td>
                            <td>2016-01-01</td>
                            <td>100</td>
                        </tr>
                        <tr>
                            <td>8</td>
                            <td class="subject">
                                <a href="#">
                                    제3회 『건강한 다비퀸』 돼지 사진 공모전 결과
                                </a>
                            </td>
                            <td>다비육종</td>
                            <td>2016-01-01</td>
                            <td>100</td>
                        </tr>
                        <tr>
                            <td>7</td>
                            <td class="subject">
                                <a href="#">
                                    대덕종돈 전문종축장 건축공사 입찰공고(첨부 입찰설명서 수정)
                                </a>
                            </td>
                            <td>다비육종</td>
                            <td>2016-01-01</td>
                            <td>100</td>
                        </tr>
                        <tr>
                            <td>6</td>
                            <td class="subject">
                                <a href="#">
                                    제3회 『건강한 다비퀸』 돼지 사진 공모전
                                </a>
                            </td>
                            <td>다비육종</td>
                            <td>2016-01-01</td>
                            <td>100</td>
                        </tr>
                        <tr>
                            <td>5</td>
                            <td class="subject">
                                <a href="#">
                                    제2회 『건강한 다비퀸』 돼지 사진 공모전
                                </a>
                            </td>
                            <td>다비육종</td>
                            <td>2016-01-01</td>
                            <td>100</td>
                        </tr>
                        <tr>
                            <td>4</td>
                            <td class="subject">
                                <a href="#">
                                    덕종돈 전문종축장 건축공사 입찰공고(첨부 입찰설명서 수정)
                                </a>
                            </td>
                            <td>다비육종</td>
                            <td>2016-01-01</td>
                            <td>100</td>
                        </tr>
                        <tr>
                            <td>3</td>
                            <td class="subject">
                                <a href="#">
                                    제3회 『건강한 다비퀸』 돼지 사진 공모전
                                </a>
                            </td>
                            <td>다비육종</td>
                            <td>2016-01-01</td>
                            <td>100</td>
                        </tr>
                        <tr>
                            <td>2</td>
                            <td class="subject">
                                <a href="#">
                                    제2회 『건강한 다비퀸』 돼지 사진 공모전
                                </a>
                            </td>
                            <td>다비육종</td>
                            <td>2016-01-01</td>
                            <td>100</td>
                        </tr>
                        <tr>
                            <td>1</td>
                            <td class="subject">
                                <a href="#">
                                    제2회 『건강한 다비퀸』 돼지 사진 공모전
                                </a>
                            </td>
                            <td>다비육종</td>
                            <td>2016-01-01</td>
                            <td>100</td>
                        </tr>
                    </tbody>
                </table>
                <div class="btn-area">
                    <p>
                        <a href="#" class="btn btn-green" role="button">글쓰기</a>
                    </p>
                </div>
                <nav class="paging">
                    <ul class="pagination">
                        <li>
                            <a href="#" aria-label="first-Previous">
                                <span aria-hidden="true">&laquo;</span>
                            </a>
                        </li>
                        <li>
                            <a href="#" aria-label="Previous">
                                <span aria-hidden="true">&lsaquo;</span>
                            </a>
                        </li>
                        <li class="active"><a href="#">1</a></li>
                        <li><a href="#">2</a></li>
                        <li><a href="#">3</a></li>
                        <li><a href="#">4</a></li>
                        <li><a href="#">5</a></li>
                        <li>
                            <a href="#" aria-label="Next" >
                                <span aria-hidden="true">&rsaquo;</span>
                            </a>
                        </li>
                        <li>
                            <a href="#" aria-label="end-Next">
                                <span aria-hidden="true">&raquo;</span>
                            </a>
                        </li>
                    </ul>
                </nav>
            </div>
		</div>
	</main>
    <?php require_once($_SERVER['DOCUMENT_ROOT'].'/inc/footer.php'); ?>
    <?php require_once($_SERVER['DOCUMENT_ROOT'].'/inc/docfoot.php'); ?>
</body>
</html>
