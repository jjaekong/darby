<?php require_once($_SERVER['DOCUMENT_ROOT'].'/en/inc/dochead.php'); ?>
<link href="/en/assets/css/sub.css" rel="stylesheet" />
<link href="/en/assets/css/business.css" rel="stylesheet" />
</head>
<body>
	<?php require_once($_SERVER['DOCUMENT_ROOT'].'/en/inc/header.php'); ?>
	<div class="page-header page-header-1">
		<div class="container">
			<ol class="breadcrumb">
				<li><a href="#"><span class="glyphicon glyphicon-home"></span></a></li>
				<li><a href="#">사업소개</a></li>
				<li class="active">제품소개</li>
			</ol>
			<hr>
			<h1>제품소개</h1>
			<p>다비육종의 사업 및 제품을 소개합니다.</p>
		</div>
	</div>
	<main id="content" tabindex="-1" class="business xperm">
		<div class="container">
			<nav class="navbar">
				<ul class="nav navbar-nav">
					<li><a href="/en/business/product/breeding_pig.php">순종</a></li>
					<li><a href="/en/business/product/darbyqueen.php">F1</a></li>
					<li class="active"><a href="/en/business/product/xperm.php">엑스펌</a></li>
					<li><a href="/en/business/product/stain_pig.php">한돈 브랜드육</a></li>
					<li><a href="/en/business/product/purchase.php">구입문의</a></li>
				</ul>
			</nav>
			<section class="sperm">
				<div class="section-header">
					<h3>엑스펌 (Xperm, 돼지 액상정액)</h3>
					<hr>
				</div>
				<p>
					다비육종의 엑스펌 시리즈는 대한민국 인공수정 시장점유율 1위를 꾸준히 지키고 있습니다.다비육종 SP(인공수정)센터는 국내 최대 규모의 웅돈을 보유하고 있으며, 최고의 인공수정 전문가 그룹으로 구성되어 있습니다. 돈군의 고위생도 유지와 더불어 지속적인 웅돈 교체를 통해 우수한 유전자원을 농장에 안전하고 신속하게 공급하고 있습니다.
				</p>
				<div class="graph">
					<img src="/en/assets/images/business/graph06.jpg" alt="연간 정액 판매량 그래프">
					<p>연간 엑스펌 판매량 추세</p>
				</div>
			</section>
			<section class="xperm-v">
				<div class="section-header">
					<h3>엑스펌-V</h3>
					<hr>
				</div>
				<h4>분만율과 산자수 증가를 위한 인공수정용 액상 정액 "엑스펌-V"</h4>
				<ul class="dot-list">
					<li>엑스펌-V(Vital)는 유전평가에서 우수한 Index 값을 나타내는 웅돈으로부터 생산됩니다.</li>
					<li>웅돈에게 필요한 추가적인 영양소를 공급함으로써 정자의 운동성과 생존율을 개선시켜 농장의 번식성적을 향상시킵니다.</li>
				</ul>
				<p class="xperm-img">
					<img src="/en/assets/images/business/xperm_v.png" alt="엑스펌-v">
				</p>
			</section>
			<section class="xperm-s">
				<div class="section-header">
					<h3>엑스펌-S</h3>
					<hr>
				</div>
				<h4>슬림한 등지방 두께를 위한 인공수정용 액상 정액 "엑스펌-S"</h4>
				<ul class="dot-list">
					<li>
						비육돈의 과도한 등지방 문제를 해결하기 위해 개발된 엑스펌-S(Slim)는 등지방 육종가가 낮은 웅돈에서 생산됩니다.
					</li>
					<li>
						두꺼운 등지방으로 인해 낮은 등급평가를 받는 농가는 엑스펌-S를 사용함으로써 출하등급을 향상시킬 수 있습니다.
					</li>
				</ul>
				<p class="xperm-img">
					<img src="/en/assets/images/business/xperm_s.png" alt="엑스펌-s">
				</p>
			</section>
			<section class="xperm-p">
				<div class="section-header">
					<h3>엑스펌-P</h3>
					<hr>
				</div>
				<h4>저지방 돈육의 생산을 위한 인공수정용 액상정액 "엑스펌-P"</h4>
				<ul class="dot-list">
					<li>
						엑스펌-P(Pietrain)를 사용해서 생산된 비육돈은 높은 정육생산량을 나타냅니다.
					</li>
					<li>
						종료교배 시 피어트레인 정액을 사용함으로써 지방함량이 낮은 비육돈을 생산할 수 있습니다.
					</li>
				</ul>
				<p class="xperm-img">
					<img src="/en/assets/images/business/xperm_p.png" alt="엑스펌-p">
				</p>
			</section>
		</div>
	</main>
    <?php require_once($_SERVER['DOCUMENT_ROOT'].'/en/inc/footer.php'); ?>
    <?php require_once($_SERVER['DOCUMENT_ROOT'].'/en/inc/docfoot.php'); ?>
</body>
</html>
